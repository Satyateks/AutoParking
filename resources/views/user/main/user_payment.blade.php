        ...
        <div class="row">
            <div class="col-lg-6 mb-4 pt-4">
                <input type="text" class="form-control" id="exampleInputEmail1" name="location" aria-describedby="emailHelp"
                    placeholder="Search for a Parking Area" {{-- value="{{ session('location') }}" --}} value="{{ $data->address }}" />
                <div id="hintBox"></div>
            ...
            <div class="col-lg-3 mb-4">
                ...
                <input type="datetime-local" class="form-control" {{-- id="start_date" --}} name="start_date"
                    aria-describedby="emailHelp" placeholder="Select Start Date" value="{{ Session('start') }}"
                    {{-- min="{{ now()->toDateString() }}" --}} onkeyup="updateURL('start_date')" />
            </div>
                ...
                <div class="row">
                    ...
                    <div class="col-lg-4 listing-column">
                        <div class="vehicle-number-box">
                            <form class="row" action="{{ route('vahicle_number_add111') }}" method="POST"
                                data-parsley-validate>
                                @csrf
                                <h6 class="mb-3"><b>Add Vehicle Number</b></h6>
                                <div class="mb-3 col-lg-8">
                                    <input type="text" name="vehicle" value="{{ Auth::user()->vehicle ?? '' }}"
                                        class="form-control" required pattern="[A-Za-z0-9]{1,16}"
                                        data-parsley-length="[8,15]" maxlength="15" minlength="8">
            ...
            ><i class="bi bi-star-fill"></i> &nbsp; --}}
              ...
              &nbsp; (4.6)
                            ...
                            <div class="total-price-detail">
                                ...
                                <div class="mb-3">
                                    ...
                                    @elseif(Session('hour') > 24  && Session('hour') < 24 * 7)
                                        <h4>
                                            <b> ${{ $data->daily * Session('days') }}</b>
                                        </h4>
                                    @elseif(Session('hour') >= 24 * 7 && Session('hour') < 24 * 30)
                                        <h4>
                                            <b> ${{ $data->weekly * Session('weeks') }}</b>
                                        </h4>
                                    @elseif(Session('hour') >= 24 * 30)
                                        <h4>
                                            <b> ${{ $data->monthly * Session('months') }}</b>
                                        </h4>
                                    @endif
                            ...
                            <div class="total-price-detail">
                                ...
                                <div class="mb-3">
                                    @if (Session('hour') > 24 && Session('hour') < 24 * 7)
                                        <h4>
                                            <b>
                                                ${{ number_format($data->daily * Session('days') * 1.18 - $data->daily * Session('days'), 2) }}</b>
                                        </h4>
                                    ...
                                    @elseif(Session('hour') >= 24 * 7 && Session('hour') < 24 * 30)
                                        <h4>
                                            <b>${{ number_format($data->weekly * Session('weeks') * 1.18 - $data->weekly * Session('weeks'), 2) }}</b>
                                        </h4>
                                    @elseif(Session('hour') >= 24 * 30)
                                        <h4>
                                            <b>${{ number_format($data->monthly * Session('months') * 1.18 - $data->monthly * Session('months'), 2) }}</b>
                                        </h4>
                                    @endif
                            ...
                            <div class="total-price-detail">
                                ...
                                <div class="mb-3">
                                    @if (Session('hour') > 24 * 30)
                                        <h4>
                                            <b>${{ number_format($data->monthly * Session('months') * ($commission->commission / 100), 2) }}</b>
                                        </h4>
                                    @elseif(Session('hour') >= 24 * 7 && Session('hour') < 24 * 30)
                                        <h4>
                                            <b>${{ number_format($data->weekly * Session('weeks') * ($commission->commission / 100), 2) }}</b>
                                        </h4>
                                    @elseif(Session('hour') > 24 && Session('hour') < 24 * 7)
                                        <h4>
                                            <b>${{ number_format($data->daily * Session('days') * ($commission->commission / 100), 2) }}</b>
                                        </h4>
                                    @else
                                        <h4>
                                            <b>${{ number_format($data->hourly * Session('hour') * ($commission->commission / 100), 2) }}</b>
                                        </h4>
                                    @endif
                            ...
                            <div class="total-price-detail">
                                ...
                                @if (Session('hour') > 24 * 30)
                                    <h4>
                                        <b>${{ number_format($data->monthly * Session('months') * (1 + $commission->commission / 100) + $data->monthly * Session('months') * 0.18, 2) }}</b>
                                    </h4>
                                @elseif(Session('hour') >= 24 * 7 && Session('hour') < 24 * 30)
                                    <h4>
                                        <b>${{ number_format($data->weekly * Session('weeks') * (1 + $commission->commission / 100) + $data->weekly * Session('weeks') * 0.18, 2) }}</b>
                                    </h4>
                                @elseif(Session('hour') > 24 && Session('hour') < 24 * 7)
                                    <h4>
                                        <b>${{ number_format($data->daily * Session('days') * (1 + $commission->commission / 100) + $data->daily * Session('days') * 0.18, 2) }}</b>
                                    </h4>
                                @else
                                    <h4>
                                        <b>${{ number_format($data->hourly * Session('hour') * (1 + $commission->commission / 100) + $data->hourly * Session('hour') * 0.18, 2) }}</b>
                                    </h4>
                                @endif
                        ...
                        @php
                            $price = 0;
                            if (Session('hour') >= 24 * 30) {
                                $price = $data->monthly * Session('months');
                            } elseif (Session('hour') < 24 * 30 && Session('hour') >= 24 * 7) {
                                $price = $data->weekly * Session('weeks');
                            } elseif (Session('hour') < 24 * 7 && Session('hour') >= 24) {
                                $price = $data->daily * Session('days');
                            } else {
                                $price = $data->hourly * Session('hours');
                            }
                            $commissionPercentage = $commission->commission ?? 0;
                            $commissionAmount = ($commissionPercentage / 100) * $price;
                            $taxamount = (18 / 100) * $price;
                            $payamount = $price + $commissionAmount + $taxamount;
                        @endphp
                        ...
                        <form id='checkout-form' method='post' action="{{ route('stripe.post') }}">
                            ...
                            <input type="hidden" name="price" value="{{ number_format($payamount, 2) }}">
                            ...
                            <button id='pay-btn' class="btn btn-success mt-3" type="button"
                                style="margin-top: 20px; width: 100%;padding: 7px;" onclick="createToken()">
