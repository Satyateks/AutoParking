        ...
        <div class="col-lg-4 listing-column">
          <div class="detail-column">
            ...
            <div class="d-flex listing-star-icon">
              ...
              <h5>
                &nbsp;
                <img src="{{asset('images_uploade/'.$data->image)}}" width="15" height="15" alt="" />
                &nbsp; Limited Space Left
              </h5>
            </div>
...
@elseif(Session('hour') >= 24 && Session('hour') < 24 * 7)
    <h4 >
        <b>
       {{$data->weekly * Session('hour')}}$
    </b>
    </h4>
@elseif(Session('hour') >= 24 * 7 && Session('hour') < 24 * 30)
    <h4>
        <b>
        {{$data->monthly * Session('hour')}}$
    </b>
    </h4>
@endif
            ...
            <div>
                <img src="{{asset('images_uploade/'.$data->image)}}" width="100%" height="250px" alt="" />
    ...
    <form action="{{ url('user_payment',$data->id) }}" method="POST">
        @csrf
        <input type="hidden" name="booking_cost" value="{{$data->daily * Session('hour')}}">
        <button type="submit" class="booking-btn mt-2">
            Book Now for ${{$data->daily * Session('hour')}}
        </button>
    </form>
@elseif(Session('hour') < 24)
    <form action="{{ url('user_payment',$data->id) }}" method="POST">
...
@elseif(Session('hour') >= 24 && Session('hour') < 24 * 7)
    <form action="{{ url('user_payment',$data->id) }}" method="POST">
        @csrf
        <input type="hidden" name="booking_cost" value="{{$data->weekly * Session('hour')}}">
        <button type="submit" class="booking-btn mt-2">
            Book Now for ${{$data->weekly * Session('hour')}}
        </button>
    </form>
@elseif(Session('hour') >= 24 * 7 && Session('hour') < 24 * 30)
    <form action="{{ url('user_payment',$data->id) }}" method="POST">
        @csrf
        <input type="hidden" name="booking_cost" value="{{$data->monthly * Session('hour')}}">
        <button type="submit" class="booking-btn mt-2">
            Book Now for ${{$data->monthly * Session('hour')}}
        </button>
    </form>
@endif
